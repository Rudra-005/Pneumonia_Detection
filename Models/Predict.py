...
# Try to import YOLOv5, but handle gracefully if not available
try:
    ...
    print("Warning: YOLOv5 not available. YOLOv5 functionality will be disabled.")

@st.cache_resource
def load_yolov5():
    """Load YOLOv5 model safely on Windows"""
    if not YOLOV5_AVAILABLE:
        print("YOLOv5 not available, returning None")
        return None
    ...
    try:
        device = select_device("0" if (TORCH_AVAILABLE and torch.cuda.is_available()) else "cpu")
...
def predict_with_yolo(yolo_model, file_or_path, conf=0.25):
    ...
    """
    if not YOLOV5_AVAILABLE or yolo_model is None:
    ...
    try:
        ...
        if isinstance(image, str):
            image = np.array(load_file_as_pil(image))
        return [], image
