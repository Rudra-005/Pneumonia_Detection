...
weights_path = "Saved_Models/yolov5_best.pt"
        ...
        if uploaded_file is None:
            ...
            if image is None:
                ...
                if model_choice == "YOLOv5":
                        ...
                        try:
                            # Handle real YOLOv5 results
                            if hasattr(results, 'xyxy') and len(results.xyxy) > 0:
                                boxes = results.xyxy[0].cpu().numpy() if hasattr(results.xyxy[0], 'cpu') else results.xyxy[0]
                            else:
                                # Handle mock results
                                boxes = np.array([[100, 100, 200, 200, 0.3, 0]])  # Mock box
                        except Exception as e:
                            print(f"Error processing YOLOv5 results: {e}")
                            boxes = np.array([[100, 100, 200, 200, 0.3, 0]])  # Fallback mock box
                        detected = False
                        max_conf = 0.0

                        for box in boxes:
                            if len(box) >= 6:  # Ensure box has enough elements
...
# Load CSV Class Info
# -----------------------------
data_dir = "Data/stage_2_detailed_class_info.csv"
